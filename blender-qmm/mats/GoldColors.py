...
class GoldColorsGroup(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not ng_gcg:
            ...
            links(gc_cg.outputs[0], group_out.inputs[3])
            links(gc_vg.outputs[0], group_out.inputs[4])
            links(gc_og.outputs[0], group_out.inputs[5])
            links(gc_ssg.outputs[0], group_out.inputs[6])
            links(gc_prtg.outputs[0], group_out.inputs[7])
            links(gc_gy.outputs[0], group_out.inputs[8])
            links(gc_gg.outputs[0], group_out.inputs[9])
            links(gc_gp.outputs[0], group_out.inputs[10])
            links(gc_hg.outputs[0], group_out.inputs[11])
