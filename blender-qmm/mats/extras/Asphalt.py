...
class QMMAsphalt(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if m_asphalt:
            ...
            m_colorramp.location = (-1100,0)
            m_colorramp.color_ramp.elements[0].color = (0.02, 0.02, 0.02, 1.0)
            m_colorramp.color_ramp.elements[0].position = 0.25
            m_colorramp.color_ramp.elements[1].color = (0.333, 0.333, 0.333, 1.0)
            ...
            m_colorramp2.location = (-1600,300)
            m_colorramp2.color_ramp.elements[0].color = (1.0, 1.0, 1.0, 1.0)
            m_colorramp2.color_ramp.elements[1].color = (0.0, 0.0, 0.0, 1.0)
            m_colorramp2.color_ramp.elements[1].position = 0.025

            #colorramp3
            m_colorramp3 = nodes.new('ShaderNodeValToRGB')
            m_colorramp3.location = (-1600,0)
            ...
            links = m_asphalt.node_tree.links

            links.new(m_mixshader.outputs[0], material_output.inputs[0])
            links.new(m_colorramp2.outputs[0], m_mixshader.inputs[0])
            ...
            links.new(m_bump2.outputs[0], m_bump.inputs[3])
            links.new(m_colorramp2.outputs[0], m_bump2.inputs[2])
            links.new(m_colorramp3.outputs[0], m_mix.inputs[0])
            links.new(m_voronoi.outputs[0], m_mix.inputs[1])
            links.new(m_noise.outputs[0], m_mix.inputs[2])
            links.new(m_voronoi2.outputs[0], m_colorramp2.inputs[0])
            links.new(m_noise2.outputs[0], m_colorramp3.inputs[0])
            links.new(m_mix2.outputs[0], m_voronoi2.inputs[0])
            links.new(m_noise3.outputs[1], m_mix2.inputs[1])
            links.new(m_mapping.outputs[0], m_noise3.inputs[0])
