...
class QMMCuttingMat(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if m_cutting_mat:
            ...
            links = m_cutting_mat.node_tree.links

            links.new(m_texscale.outputs[0], m_multiply.inputs[0])
            links.new(m_texscale.outputs[0], m_bricktexture.inputs[4])
            links.new(m_mapscale.outputs[0], m_mapping.inputs[3])
            links.new(m_textcoord.outputs[2], m_mapping.inputs[0])
            links.new(m_multiply.outputs[0], m_bricktexture2.inputs[4])
            links.new(m_mapping.outputs[0], m_bricktexture.inputs[0])
            links.new(m_mapping.outputs[0], m_bricktexture2.inputs[0])
            links.new(m_bricktexture.outputs[0], m_add.inputs[1])
            links.new(m_bricktexture2.outputs[0], m_add.inputs[2])
            links.new(m_add.outputs[0], m_mix.inputs[0])
            links.new(m_mix.outputs[0], BSDF.inputs[0])
