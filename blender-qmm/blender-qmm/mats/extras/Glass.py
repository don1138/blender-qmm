...
class QMMGlass(bpy.types.Operator):
    ...
    def make_shader(self):
        ...
        m_fresnel.inputs["IOR"].default_value = 40

        # glossy
        m_glossy = make_node(nodes, 'ShaderNodeBsdfAnisotropic', -600, -120)
        m_glossy.inputs["Color"].default_value = (1, 1, 1, 1)
        m_glossy.inputs["Roughness"].default_value = 0
        m_glossy.distribution = 'MULTI_GGX'

        # transparent
        m_transparent = make_node(nodes, 'ShaderNodeBsdfTransparent', -600, -300)
        m_transparent.inputs["Color"].default_value = (1, 1, 1, 1)
        ...
        m_noise.inputs["Scale"].default_value = 3.14159
        m_noise.inputs["Detail"].default_value = 0
        m_noise.inputs["Roughness"].default_value = 1

        links = m_glass.node_tree.links.new

        links(m_noise.outputs["Fac"], m_mixrgb.inputs[7])
        links(m_mixrgb.outputs[2], m_transparent.inputs["Color"])

        links(m_transparent.outputs["BSDF"], m_mix2.inputs[2])
        links(m_glossy.outputs["BSDF"], m_mix2.inputs[1])
        links(m_fresnel.outputs["Fac"], m_mix2.inputs["Fac"])
        links(m_light_path.outputs["Is Diffuse Ray"], m_add2.inputs[1])
        links(m_light_path.outputs["Is Shadow Ray"], m_add2.inputs[0])
        links(m_light_path.outputs["Is Glossy Ray"], m_add.inputs[1])
        links(m_add2.outputs["Value"], m_add.inputs[0])
        links(m_transparent.outputs["BSDF"], m_mix.inputs[2])
        links(m_mix2.outputs["Shader"], m_mix.inputs[1])
        links(m_add.outputs["Value"], m_mix.inputs["Fac"])
